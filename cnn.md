...
* 결론: 합성곱 층에서는 여러 개의 필터(커널)를 사용하여 이미지의 특징을 추출하는데, 각 필터는 서로 다른 특징을 학습하며 필터의 개수만큼 특성 맵(Feature Map)이 생성된다.
* 이 피처맵에는 양수와 음수 값이 모두 포함될 수 있다.

#### 2️⃣ ReLU 활성화 함수 적용
* 컨볼루션 연산을 거친 값들은 **음수일 수도 있고 양수일 수도 있다**.
* 하지만 **음수 값은 특징을 나타내지 않는 경우가 많기 때문에 0으로 변환**한다.
* 활성화 함수를 통해 "활성화된 피처맵"을 생성한다.
...
#### 그렇다면 어디에서 학습이 이루어지는가?

CNN에서의 학습은 크게 두 가지 단계로 진행된다: **순전파(Forward Propagation)** 과정, 그리고 **역전파(Back Propagation)** 과정

순전파에서는:
- 합성곱 층의 필터(커널) 가중치들이 학습 파라미터 (초기에는 필터의 가중치가 랜덤값으로 초기화됨)
- 입력 이미지가 들어오면 현재의 필터 가중치로 특징을 추출 (예를 들어 3x3 필터라면 9개의 가중치 값이 있고, 이 값들이 학습 대상)
- 필터를 통해 나온 특성 맵은 ReLU와 맥스 풀링을 거침
- 최종적으로 완전 연결 층에서 분류 결과 출력
- 이 출력값과 실제 정답 레이블의 차이(오차)를 계산

역전파에서는:
- 계산된 오차를 기반으로 손실 함수의 기울기(gradient) 계산
- 이 기울기를 사용해 네트워크의 모든 가중치를 업데이트 (앞서 말했던 9개의 가중치를 역전파 과정에서 조금씩 수정됨)
- 합성곱 층의 필터 가중치들이 조금씩 수정됨
- 이 과정을 통해 필터들이 중요한 특징(엣지, 텍스처 등)을 잡아내고 감지하도록 발전하고 학습
...
이 세 가지 요소들이 역전파를 통해 지속적으로 업데이트되면서 이루어진다. 맥스 풀링은 학습 파라미터가 없고, ReLU는 단순히 비선형 함수를 적용하는 것이므로 실제 학습이 일어나지 않는다. 학습 과정의 흐름을 보면

(1) 순전파: 입력 이미지 → 합성곱 연산 → ReLU → 맥스 풀링. 이 과정 여러 번 반복 완전 연결 층 통과 최종 예측 결과 출력

(2) 오차 계산: 예측값과 실제 레이블 비교 손실 함수(예: 교차 엔트로피) 계산

(3) 역전파: 손실에 대한 그래디언트 계산 경사하강법으로 모든 가중치와 편향 업데이트

이 과정을 통해 네트워크가 점진적으로 학습 이 전체 과정을 정해진 에포크(epoch) 수만큼 반복하면서 네트워크가 점점 더 정확한 예측을 할 수 있도록 발전한다.

---
#### Note. 합성곱 층의 필터(커널) 가중치들이 학습 파라미터이므로 실제 수학에서 다루는 컨볼루션 연산과 딥러닝 컨볼루션 연산에서 차이가 존재한다.

- 수학에서의 컨볼루션 연산은 신호 처리를 위해 정의된 것으로, 필터(커널)를 뒤집어서 사용한다.
- 딥러닝에서는 컨볼루션 연산을 패턴을 감지하는 학습 가능한 연산으로 사용하기 (특징 추출이 중요하기 때문에) 필터를 뒤집지 않는다.
- 딥러닝에서는 필터(커널)가 고정된 것이 아니라 학습되는 가중치이다.
- 즉, CNN에서 사용하는 연산은 수학적 컨볼루션이 아니라, **크로스 코릴레이션(cross-correlation)**과 같은 방식으로 동작한다.
